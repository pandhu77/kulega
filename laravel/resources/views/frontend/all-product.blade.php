...
@section('content')
    ...
    <div class="container">
        <div class="col-md-12">
            <img class="img-responsive" src="{{asset('assets/img/header-product.png') }}" alt="" style="width: 100%;">
        </div><br>
        ...
        <h5 class="judul-sub-pro">  <a href="{{url('product/'.$categs->kateg_url)}}" style="color:#000;">VIEW ALL {{$categs->kateg_name}}</a></h5>
            <ul class="topnav">

            <?php if(count($categparent) > 0){?>
              @foreach($categparent as $parent)
                @if($categs->kateg_id==$parent->kateg_parent)
                <li id="{{$parent->kateg_id}}"><a href="{{url('product/'.$parent->kateg_url)}}">{{$parent->kateg_name}}</a>
              ...
              @foreach($categall as $categs)
                <?php if(in_array($categs->kateg_id,explode(',',$product->prod_category))){?>
                       ...
                       <div class="col-md-4 col-xs-6 product-loop">
                          <div class="card">
                              <div class="hovereffect1" style="background-image:url({{$product->front_image}});overflow:hidden;">
              ...
              <ul class="topnav">
                <?php if(count($categparent) > 0){?>
                  ...
                  <li>
                    ...
                    <a href="{{url('product/'.$parent->kateg_url)}}" style="margin-right:20px;">{{$parent->kateg_name}}</a>
                    @foreach($categparent2 as $parent2)
                    @if($parent->kateg_id==$parent2->kateg_parent )
                    <ul>
                      @foreach($categparent2 as $parent2)
